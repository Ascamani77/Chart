...
import { formatPrice, priceStepFor } from '../utils/formatPrice';
...
const AlertModal: React.FC<AlertModalProps> = ({ symbol, onClose, onCreate, lastPrice = 0 }) => {
  ...
  // Form State
  const [price, setPrice] = useState<string>(formatPrice(lastPrice, symbol));
  const [alertName, setAlertName] = useState<string>('');
  const [message, setMessage] = useState<string>(`${symbol} Crossing ${formatPrice(lastPrice, symbol)}`);
  ...
  const TabButton = ({ id, label, count }: { id: any, label: string, count?: number }) => (
    <button
      ...
      className={`relative pb-2 text-[13px] font-bold transition-colors ${activeTab === id ? 'text-white' : 'text-[#787b86] hover:text-gray-300'
        }`}
    >
                  ...
                  <div className="flex space-x-2">
                    ...
                    <div className="flex-1 bg-black border border-[#2962ff] rounded-md flex items-center px-2 py-2 min-w-0 ring-1 ring-[#2962ff]/30 shadow-[0_0_10px_rgba(41,98,255,0.15)]">
                      ...
                      <div className="flex flex-col -space-y-1 text-[#787b86] ml-0.5 shrink-0">
                        <button onClick={() => setPrice((p) => formatPrice(parseFloat(p) + priceStepFor(symbol), symbol))} className="hover:text-white"><ChevronDown size={10} className="rotate-180" /></button>
                        <button onClick={() => setPrice((p) => formatPrice(parseFloat(p) - priceStepFor(symbol), symbol))} className="hover:text-white"><ChevronDown size={10} /></button>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex-1">
                  <div className="flex bg-[#131722] rounded-md p-0.5 border border-[#363a45]">
                    <button
                      ...
                      className={`flex-1 py-1 text-[12px] font-bold rounded transition-all ${triggerMode === 'once' ? 'bg-[#2a2e39] text-white shadow-sm' : 'text-[#787b86] hover:text-gray-200'
                        }`}
                    >
                      Only once
                    </button>
                    <button
                      ...
                      className={`flex-1 py-1 text-[12px] font-bold rounded transition-all ${triggerMode === 'every' ? 'bg-[#2a2e39] text-white shadow-sm' : 'text-[#787b86] hover:text-gray-200'
                        }`}
                    >
                      Every time
                    </button>
              ...
              {notifications.map((item) => (
                <div key={item.id} className="flex flex-col space-y-0.5">
                  <div className="flex items-center group cursor-pointer" onClick={() => toggleNotification(item.id)}>
                    <div className={`w-4 h-4 rounded-[2px] border flex items-center justify-center mr-3 transition-colors shrink-0 ${item.active ? 'bg-[#2962ff] border-[#2962ff] text-white' : 'bg-transparent border-[#434651] group-hover:border-gray-400'
                      }`}>
