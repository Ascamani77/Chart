...
import { Shapes, PenTool } from 'lucide-react';
import { ToolType } from '../../icons/toolTypes';

interface GeometricShapesToolsProps {
  activeTool: ToolType;
  onActivate: (tool: ToolType) => void;
}

const GeometricShapesTools: React.FC<GeometricShapesToolsProps> = ({ activeTool, onActivate }) => {
  const tools: { icon: React.ReactNode, id: ToolType }[] = [
    { icon: <Shapes size={24} />, id: 'shapes' as ToolType },
    { icon: <PenTool size={24} />, id: 'brush' as ToolType },
  ];
...
export default GeometricShapesTools;
